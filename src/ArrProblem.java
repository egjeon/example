...
public class ArrProblem {
    public static int [] solution(String [] cpr){
        ...
        String[] basic_order = {"check", "call", "pressure", "respiration", "repeat"}; //주어진순서
        for(int i = 0; i < cpr.length; i++){     //{"call", "respiration", "repeat", "check", "pressure"};
            for(int j = 0; j < basic_order.length; j++){
                if(basic_order[j].equals(cpr[i])){
                    answer[i] =j+1;
                    break;
                }
            }
