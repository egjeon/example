import java.util.Arrays;

//배열 만들기 5
/*문자열 배열 intStrs와 정수 k, s, l가 주어집니다.
 *intStrs의 원소는 숫자로 이루어져 있습니다.
 *배열 intStrs의 각 원소마다 s번 인덱스에서 시작하는 길이 l짜리 부분 문자열을 잘라내 정수로 변환합니다.
 *이때 변환한 정수값이 k보다 큰 값들을 담은 배열을 return 하는 solution 함수를 완성해 주세요.
 */
public class MakeArray5 {
    static int[] solution(String[] intStrs, int k, int s, int l) {
        int[] answer2 = new int[intStrs.length];
        int[] answer = {};
        int h = 0;
        for(int i=0; i<intStrs.length; i++){
            // intStrs의 인덱스의 값을 하나씩 가져와서
            String a =intStrs[i];
            // 그거를 s번째부터 l짜리 까지 짤라
            String b = a.substring(s,s+l);
            // 그리고 그게 k보다 큰 값인지 보고 k보다 크면 answer에 담아
            if(Integer.parseInt(b) > k){
                answer2[i] += Integer.parseInt(b);
                h++;
            }
        }
        answer = new int[h];
        int m = 0;
        int n = 0;
        // temp = [56789,0, 99999]

        while(m < answer2.length){
            if(answer2[m] > 0){
                answer[n++] = answer2[m];
            }
            m++;
        }

        return answer;
    }

    public static void main(String[] args) {
        System.out.println(Arrays.toString(solution(new String []{"0123456789","9876543210","9999999999999"}, 50000,5,5)));
    }
}